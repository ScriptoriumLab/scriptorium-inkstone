...
namespace modian::inkstone::service {
	...
	core::protocol::ui::v1::user_action ui_protocol_service::parse_user_action_response(const std::string& response) {
		core::protocol::ui::v1::user_action action{core::protocol::ui::v1::action_type::UNKNOWN, 0};

		try {
			const auto j = nlohmann::json::parse(response);

			if (const std::string type_str = j.value("type", "Unknown"); type_str == "SelectCandidate") {
				action.type = core::protocol::ui::v1::action_type::SELECT_CANDIDATE;
				action.payload = j.value("payload", 0);
			} else if (type_str == "PageNext") {
				action.type = core::protocol::ui::v1::action_type::PAGE_NEXT;
			} else if (type_str == "PagePrev") {
				action.type = core::protocol::ui::v1::action_type::PAGE_PREV;
			}
