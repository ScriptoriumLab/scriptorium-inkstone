...
namespace scriptorium::inkstone {
	...
	server::server(const manager::EngineDetail& engine_detail) {
		...
		auto ui_protocol_ipc_server = felt::infra::ipc::ipc_server_factory::create_async_ipc_server(UI_PROTOCOL_PIPE_NAME);
		ui_protocol_ipc_server->set_message_handler([this](std::string msg) {
			auto action = felt::service::ui_protocol_service::parse_user_action_response(msg);

			if (action.type == felt::core::protocol::ui::v1::action_type::SELECT_CANDIDATE) {
				size_t index = action.payload;

                felt::core::logger_service::logger()->info("UI Selected Candidate: {}", index);

				const auto candidate = session_orchestrator_->select_candidate(index);
				if (!candidate.word.empty()) {
                    ...
                    felt::core::logger_service::logger()->info("TODO: Commit text to Brush: {}", candidate.word);
				}
			}
		});
